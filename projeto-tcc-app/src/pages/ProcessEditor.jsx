...
const ProcessEditor = () => {
  ...
  const handleUINodeAdd = (nodeData, parentId = null) => {
    ...
    if (result.success && result.newNode) {
      ...
      toast({ title: "Node Added", description: `${result.newNode.presentationName} (${result.newNode.type || 'No Type'}) was successfully added.` });
  ...
  const handleUINodeDelete = useCallback((id) => {
    const deletedNodeName = findNode(id)?.presentationName || "Node";
    deleteNode(id);
    if (selectedNodeId === id || (selectedNodeId && nodes.find(n => n.id === selectedNodeId)?.parentId === id) ) {
      setSelectedNodeId(null);
    }
    toast({ title: "Node Deleted", description: `${deletedNodeName} and its children were deleted.`, variant: "destructive" });
  }, [selectedNodeId, deleteNode, findNode, nodes, toast]); 
  
  const handleUIDeleteAllNodes = useCallback(() => {
    deleteAllNodes();
    setSelectedNodeId(null);
    toast({ title: "All Nodes Deleted", description: "The entire process model has been cleared.", variant: "destructive" });
  }, [deleteAllNodes, toast]);

  const selectedNode = findNode(selectedNodeId) || null;

  const onDragStart = (event) => {
    handleDragStartLogic(event.active.id);
  };
  ...
  return (
    <DndContext 
      ...
      onDragEnd={onDragEnd}
    >
      <div className="flex-1 grid grid-cols-1 md:grid-cols-[1fr_380px] gap-6 p-6 bg-gray-100 dark:bg-gray-900 h-[calc(100vh-4rem)]">
        <motion.div
        ...
        >
          <ProcessTreeView 
            ...
            onAddRootNode={() => {
              if (hasRootNode) {
                toast({ title: "Action Denied", description: "A root 'Process' node already exists.", variant: "destructive" });
                return;
              }
        ...
        >
          ...
          <motion.div
            whileHover={{ scale: isSaving ? 1 : 1.03, y: isSaving ? 0 : -2 }}
          ...
          >
            <Button 
              ...
              disabled={isSaving}
              className="w-full bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-600 hover:to-indigo-700 text-white font-bold py-3 text-base shadow-lg rounded-lg disabled:opacity-70 disabled:cursor-not-allowed"
            >
